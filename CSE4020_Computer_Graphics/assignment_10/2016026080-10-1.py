...
def render():
    global gCamAng, gCamHeight, z1ang_Int, xang_Int, z2ang_Int
    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
    glEnable(GL_DEPTH_TEST)
    ...
    '''
    z1ang = np.radians(z1ang_Int)
    xang = np.radians(xang_Int)
    z2ang = np.radians(z2ang_Int)
    M = np.identity(4)

    Rz1 = np.array([[np.cos(z1ang), -np.sin(z1ang), 0],
                   [np.sin(z1ang), np.cos(z1ang), 0],
                   [0, 0, 1]])
    Rx = np.array([[1, 0, 0],
                   [0, np.cos(xang), -np.sin(xang)],
                   [0, np.sin(xang), np.cos(xang)]])
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480, 480,'2016026080-10-1', None,None)
    if not window:
        glfw.terminate()
        return
