...
racingtest(void)
{
  ...
  for (i = 0; i < NUM_THREAD; i++){
    if (thread_create(&threads2[i], racingthreadmain, (void*)i) != 0){
      printf(1, "panic at thread_create\n");
      return -1;
    }
  }
  for (i = 0; i < NUM_THREAD; i++){
    if (thread_join(&threads2[i], &retval) != 0 || (int)retval != i+1){
        printf(1, "%d %d\n", i, (int)retval);
        if((int)retval!=i+1){
...
basictest(void)
{
  ...
  threads[0].id=0;
  for (i = 0; i < NUM_THREAD; i++){
    if (thread_create(&threads2[i], basicthreadmain, (void*)i) != 0){
      printf(1, "panic at thread_create\n");
      return -1;
    }
  }
...
jointest1(void)
{
  ...
  for (i = 1; i <= NUM_THREAD; i++){
    if (thread_create(&threads2[i-1], jointhreadmain, (void*)i) != 0){
      printf(1, "panic at thread_create\n");
      return -1;
    }
  }
  ...
  for (i = 1; i <= NUM_THREAD; i++){
    if (thread_join(&threads2[i-1], &retval) != 0 || (int)retval != i * 2 ){
      printf(1, "panic at thread_join\n");
      return -1;
    }
  }
...
stresstest(void)
{
  ...
  for (n = 1; n <= nstress; n++){
    if (n % 1000 == 0)
      printf(1, "%d\n", n);
    for (i = 0; i < NUM_THREAD; i++){
      if (thread_create(&threads2[i], stressthreadmain, (void*)i) != 0){
        printf(1, "panic at thread_create\n");
        return -1;
      }
    }
    for (i = 0; i < NUM_THREAD; i++){
      if (thread_join(&threads2[i], &retval) != 0){
        printf(1, "panic at thread_join\n");
        return -1;
      }
    }
